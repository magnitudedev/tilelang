...
def test_short_constant_loops_are_unrolled_by_default():
    kernel = tilelang.compile(row_sums(), target="metal", execution_backend="torch")
...
def test_caller_pass_configuration_takes_precedence():
    kernel = tilelang.compile(row_sums(), target="metal", execution_backend="torch", pass_configs=ROLLED)
    body = kernel_body(kernel)
    assert len(re.findall(r"\bfor \(", body)) == 1
