...
def test_memory_and_disk_hits_reuse_the_cached_source(cache_root, lowerings):
    ...
    for name in ("device_kernel.metal", "host_kernel.metal", "params.json", "launch.json", "manifest.json"):
...
fixture.run(kernel)
print(json.dumps(kernel.adapter.launch_metadata))
"""
    ...
    assert json.loads(result.stdout.strip().splitlines()[-1]) == first.adapter.launch_metadata
