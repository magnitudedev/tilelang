...
def _elementwise(size):
    """Two BF16 operands combined in FP32 and stored back as BF16."""

    @T.prim_func
    def main(
        A: T.Tensor((size,), "bfloat16"),
        B: T.Tensor((size,), "bfloat16"),
        C: T.Tensor((size,), "bfloat16"),
    ):
        for i in T.Parallel(size):
            a, b = A[i].astype("float32"), B[i].astype("float32")
            C[i] = (a * b + T.float32(1)).astype("bfloat16")
...
@pytest.mark.parametrize("factory", [_widen, _tile_copy, _elementwise])
...
@tilelang.testing.requires_llvm
def test_llvm_bf16_elementwise_is_numerically_correct():
    size = 64
    kernel = tilelang.compile(_elementwise(size), target="llvm")
    a = torch.randn(size, dtype=torch.bfloat16)
    b = torch.randn(size, dtype=torch.bfloat16)
    c = torch.empty(size, dtype=torch.bfloat16)
    kernel(a, b, c)
    expected = (a.to(torch.float32) * b.to(torch.float32) + 1).to(torch.bfloat16)
