...
def test_build_prim_module_reuses_one_private_schedule_at_multiple_call_sites():
    ...
    module = T.build_prim_module("main", parameters, body, {"copy_schedule": schedule})
...
@pytest.mark.parametrize("kind", ["arity", "buffer_dtype", "scalar"])
def test_private_reference_validates_call_signature(kind):
    parameters = (("A", T.Tensor((8,), "float32")), ("n", tvm.tirx.Var("n", "int32")))
    private = T.build_prim_func("private", parameters, lambda A, n: None)

    def body(refs, A, n):
        if kind == "arity":
            refs["private"](A)
        elif kind == "buffer_dtype":
            refs["private"](T.view(A, shape=(8,), dtype="int32"), n)
        else:
            refs["private"](A, A)

    with pytest.raises(TypeError):
        T.build_prim_module("main", parameters, body, {"private": private})
