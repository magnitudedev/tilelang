...
CodeGenTileLangMetal::CodeGenTileLangMetal(Target target) : target_(target) {
  ...
  // Every kernel of a module is generated by its own CodeGenTileLangMetal
  // and the per-kernel sources are concatenated into one module source, so
  // the shared argument union must tolerate repeated definitions.
  decl_stream << "#ifndef __TVM_ARG_UNION_DEFINED\n"
              << "#define __TVM_ARG_UNION_DEFINED\n"
              << "union __TVMArgUnion {\n"
              << " int v_int[2];\n"
              << "};\n"
              << "#endif\n\n";
...
void CodeGenTileLangMetal::VisitStmt_(const AllocBufferNode *op) {
  ...
  if (scope == "metal.cooperative_tensor") {
    ...
    // Layout inference owns bank mapping and allocation extent. Appending
    // bytes here changes no address or bank, and rejects exact-capacity tiles.
